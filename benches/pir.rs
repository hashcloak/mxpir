...
fn get_oracle(code: &BatchCode<usize, Element>, rng: &mut Rng) 
    -> (Vec<Vec<Tuple<usize, Element>>>, Vec<(u64, u64)>) {
    ...
    let sizes: Vec<(u64, u64)> = oracle.iter()
        .map(|vec| (vec.len() as u64, mem::size_of::<(usize, Element)>() as u64))
        .collect();
    ...
    c.bench_function_over_inputs(
        ...
        |b, &&k| {
            ...
            b.iter(|| {
                ...
                MultiPirServer::new(&buckets[..], ALPHA, DIM);
            });
        },
        &BATCH_SIZES,
    );
...
fn query_cuckoo(c: &mut Criterion) {
    c.bench_function_over_inputs(
        ...
        |b, &&k| {
            ...
            // Create the client
            let client = MultiPirClient::new(&sizes, ALPHA, DIM);
...
fn reply_cuckoo(c: &mut Criterion) {
    c.bench_function_over_inputs(
        ...
        |b, &&k| {
            ...
            let client = MultiPirClient::new(&sizes, ALPHA, DIM);
            let server = MultiPirServer::new(&oracle, ALPHA, DIM);
